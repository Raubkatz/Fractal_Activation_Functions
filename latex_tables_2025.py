...
def plot_vderiv_vs_metric(df, output_dir, metric="accuracy"):
    ...
    for ds in datasets:
        ...
        for (optimizer, activation), group_df in group_oa:
            ...
            ax.plot(
                group_df_sorted["vderiv"],
                group_df_sorted[metric],
                marker=mrk,
                color=clr,
...
def generate_latex_tables(df, output_dir):
    """
    Creates two LaTeX tables for each dataset in `df`:
      1) Table for fractional optimizers (i.e., those starting with 'F')
      2) Table for non-fractional optimizers

    We list columns: [Optimizer, Activation, vderiv, accuracy].

    The tables are saved to:
      <output_dir>/<dataset>/fractional_table.tex
      <output_dir>/<dataset>/non_fractional_table.tex

    Parameters
    ----------
    df : pd.DataFrame
        Must contain columns: [dataset, optimizer, activation, vderiv, accuracy].
    output_dir : str
        The directory where the LaTeX tables will be stored.
    """
    # Ensure the top-level directory for LaTeX tables exists
    latex_base = Path(output_dir)
    ...
    for ds in datasets:
        ...
        ds_folder = latex_base / ds
        ds_folder.mkdir(parents=True, exist_ok=True)

        # Separate fractional vs. non-fractional
        df_frac = df_ds[df_ds["optimizer"].str.startswith('F')]
        df_non_frac = df_ds[~df_ds["optimizer"].str.startswith('F')]

        # Create two tables if they have data
        if not df_frac.empty:
            frac_tex = _build_latex_table(
                df_frac,
                caption=f"Fractional Optimizers for {ds}",
                label=f"tbl:frac_{ds}"
            )
            with open(ds_folder / "fractional_table.tex", "w", encoding="utf-8") as f:
                f.write(frac_tex)

        if not df_non_frac.empty:
            non_frac_tex = _build_latex_table(
                df_non_frac,
                caption=f"Non-Fractional Optimizers for {ds}",
                label=f"tbl:nonfrac_{ds}"
            )
            with open(ds_folder / "non_fractional_table.tex", "w", encoding="utf-8") as f:
                f.write(non_frac_tex)


def _build_latex_table(df_in, caption="", label=""):
    """
    Helper function that takes in a DataFrame with columns
    [optimizer, activation, vderiv, accuracy] and returns a string
    containing the LaTeX code for a tabular environment.

    Parameters
    ----------
    df_in : pd.DataFrame
        DataFrame must contain columns: [optimizer, activation, vderiv, accuracy].
    caption : str
        Caption to appear under the table.
    label : str
        Label for referencing the table in LaTeX (e.g., \label{...}).

    Returns
    -------
    str
        The LaTeX string representing the table.
    """
    # We'll keep these columns in a specific order
    columns_order = ["optimizer", "activation", "vderiv", "accuracy"]
    # Filter to keep only columns that exist in df_in
    columns_used = [col for col in columns_order if col in df_in.columns]

    # Sort by (optimizer, activation, vderiv) for consistency
    df_sorted = df_in.sort_values(by=["optimizer", "activation", "vderiv"])

    # Build the table header
    table_header = r"\begin{table}[H]" + "\n"
    table_header += r"\centering" + "\n"
    table_header += r"\begin{tabular}{llll}" + "\n"  # 4 columns, all left-aligned (adjust as needed)
    table_header += r"\hline" + "\n"

    # Column names
    col_line = " & ".join(columns_used) + r" \\"
    table_header += col_line + "\n"
    table_header += r"\hline" + "\n"

    # Build the table rows
    table_rows = ""
    for idx, row in df_sorted.iterrows():
        row_values = [str(row[c]) for c in columns_used]
        row_line = " & ".join(row_values) + r" \\"
        table_rows += row_line + "\n"

    table_footer = r"\hline" + "\n"
    table_footer += r"\end{tabular}" + "\n"
    table_footer += f"\\caption{{{caption}}}\n"
    table_footer += f"\\label{{{label}}}\n"
...
if __name__ == "__main__":
    ...
    source_dir = "results_10_runs"  # Where your merged JSON files live
    output_plots_dir = "plots"  # We'll place generated figures here
    output_latex_dir = "latex_tables"  # We'll place LaTeX tables here
    ...
    #     (B) Generate LaTeX
    # ===========================
    # We'll create separate tables for fractional vs. non-fractional results
    # for each dataset, placed in "latex_tables/<dataset>/" subfolders.
    generate_latex_tables(df_all, output_latex_dir)

    print("All plots and LaTeX tables generated.")
