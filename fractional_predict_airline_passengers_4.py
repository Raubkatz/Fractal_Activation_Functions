...
##############################################################################
FRAC_DERIV = 1.65 #1.25 works fine
...
#OPTIMIZER_FN = tf.keras.optimizers.RMSprop(learning_rate=0.1)           # RMSprop optimizer
#OPTIMIZER_FN = tf.keras.optimizers.Adamax(learning_rate=0.002)
EPOCHS = 200 #100 works great
...
# Assuming ACTIVATION_FN and OPTIMIZER_FN are defined elsewhere in your code.
def build_model_2layer(input_dim):
    """
    LSTM-based model using a fractal activation + fractional RMSProp with an added LSTM layer.
    """
    model = tf.keras.Sequential()
    # First LSTM layer
    model.add(
        tf.keras.layers.LSTM(
            48, activation=ACTIVATION_FN, input_shape=(input_dim, 1), return_sequences=True
        )
    )
    # Second LSTM layer with 12 neurons
    model.add(
        tf.keras.layers.LSTM(
...
def main():
    ...
    model = build_model_2layer(input_dim=WINDOW_SIZE)
    history = model.fit(
        X_train, y_train,
        epochs=EPOCHS,
