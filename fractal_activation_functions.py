...
# Blended function with tanh and cosine components with exponential decay using TensorFlow
def new_fav_fractal_function(x, a=0.5, b=3, c=0.5, d=8, n_terms=75, zeta=0.2666):

    # Initialize output tensor
    w = tnp.zeros_like(x)

    for n in range(n_terms):
        # Exponentially decaying cosine part using TensorFlow and tnp
        decay = tnp.exp(-tnp.abs(x) * 0.5)
        w += zeta * (0.05 * tnp.tanh(tnp.pi * x * a) + (c ** n) * tnp.cos((d ** n) * tnp.pi * x) * decay)
...
# Custom activation function: Weierstrass-Mandelbrot function using tnp
def weierstrass_mandelbrot_function_discard(x, a=0.5, b=3, n_terms=100):
    # Ensure x is float64 for all calculations
    x = tf.cast(x, tf.float64)

    # Calculate the Weierstrass-Mandelbrot function in float64
    w = tf.zeros_like(x, dtype=tf.float64)
    for n in range(n_terms):
        w += (a ** n) * tf.cos((b ** n) * tnp.pi * x)

    # Mandelbrot-like modulation
    combined_function = w * tnp.exp(-0.75 * tnp.abs(x)) + tnp.sin(2 * tnp.pi * x)
...
# Custom activation function: Modulated Weierstrass function using tnp
def modified_weierstrass_function_old(x, a=0.5, b=3, n_terms=100):
    ...
    # Calculate the modified Weierstrass function
    w = tf.zeros_like(x, dtype=tf.float64)
    for n in range(n_terms):
        w += ((-1) ** n) * (a ** n) * tf.cos((b ** n) * tnp.pi * x)
