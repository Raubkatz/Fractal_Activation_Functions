...
# ---------------------------
# NEW: timing JSON utilities
# ---------------------------
def ensure_parent_dir(path_str: str):
    Path(os.path.dirname(path_str)).mkdir(parents=True, exist_ok=True)

def save_json_silent(target_path, data):
...
def train_and_test_(results_dir, data_name, neurons, batch, ep, act, op, run_nrs=1, fr_vderivs=[1.0], seed_val=None):
    ...
    # ----------------------------------------------------------------------
    # NEW: timing log file (separate folder, separate JSON)
    # ----------------------------------------------------------------------
    time_folder_str = results_dir + "/" + "time_analysis/" + data_name
    time_file_name = optimizer_name + "_" + activation_name + "_" + data_name + "_times.json"
    time_target_path = time_folder_str + "/" + time_file_name
    Path(time_folder_str).mkdir(parents=True, exist_ok=True)

    if not os.path.exists(time_target_path):
        time_data_init = {
            "dataset": data_name,
            "neurons": neurons,
            "batch size": batch,
            ...
            "test runs": run_nrs,
            "optimizer": optimizer_name,
            "timings": []  # list of vderiv blocks
        }
        save_json_silent(time_target_path, time_data_init)
    ...
    for v in vderivs:
        ...
        # ------------------------------------------------------------------
        # NEW: timing container for this vderiv
        # ------------------------------------------------------------------
        time_v = {
            "vderiv": float(v),
            "avg": {},
        ...
        for n in ns:
            ...
            t_split_scale = time.perf_counter() - t0
            ...
            # Keep your existing "testing_time" meaning = predict-only
            t0_pred = time.perf_counter()
            y_pred = model.predict(X_test)
            t_predict = time.perf_counter() - t0_pred
            testing_time = t_predict  # preserve your existing semantics
            ...
            # NEW: total run time (everything in loop)
            t_total_run = time.perf_counter() - t0_total
            t_predict_and_metrics = t_predict + t_metrics
            ...
            # ------------------------------------------------------------------
            # NEW: per-run timing record (separate JSON)
            # ------------------------------------------------------------------
            time_v["runs"].append({
                "run #": int(n),
                "t_total_run": float(t_total_run),
                "t_split_scale": float(t_split_scale),
                "t_compile": float(t_compile),
                "t_fit": float(t_fit),
                "t_evaluate": float(t_evaluate),
                "t_predict": float(t_predict),
                "t_metrics": float(t_metrics),
                "t_predict_and_metrics": float(t_predict_and_metrics),
            })
        ...
        # ------------------------------------------------------------------
        # NEW: timing averages + save timing JSON
        # ------------------------------------------------------------------
        def avg_of(key):
            return float(np.mean([r[key] for r in time_v["runs"]])) if time_v["runs"] else None

        time_v["avg"] = {
            "t_total_run": avg_of("t_total_run"),
            "t_split_scale": avg_of("t_split_scale"),
            "t_compile": avg_of("t_compile"),
            "t_fit": avg_of("t_fit"),
            "t_evaluate": avg_of("t_evaluate"),
            "t_predict": avg_of("t_predict"),
            "t_metrics": avg_of("t_metrics"),
            "t_predict_and_metrics": avg_of("t_predict_and_metrics"),
        }

        time_json = load_json(time_target_path)
        time_json["timings"].append(time_v)
        save_json_silent(time_target_path, time_json)
