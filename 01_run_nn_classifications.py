...
def binary_model(n_features, n_classes, neurons, activation, seed=42):
...
def groups_model(n_features, n_classes, neurons, activation, seed=42):
    initializer = tf.keras.initializers.GlorotUniform(seed=seed)
    model = tf.keras.Sequential([
        tf.keras.layers.Dense(neurons, activation=activation, kernel_initializer=initializer, input_dim=n_features),
...
def train_and_test_(results_dir, data_name, neurons, batch, ep, act, op, run_nrs=1, fr_vderivs=[1.0], seed_val=None):
    ...
    for v in vderivs:
        ...
        for n in ns:
            ...
            # Define the model
            if n_classes == 2:
                model = binary_model(n_features, n_classes, neurons, act_fn[1])#, seed=n)

            elif n_classes > 2:
                model = groups_model(n_features, n_classes, neurons, act_fn[1])#, seed=n)
...
target_dir = f"speed_run_{runs}runs_seedval{seed_val}_"
